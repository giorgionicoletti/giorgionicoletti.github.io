...
<body>
    	...
    	<script>
            ...
            var blueIcon = new MarkerIcon({iconUrl: './leaflet_images/marker-icon_blue.png'}),
                ...
                aquaIcon = new MarkerIcon({iconUrl: './leaflet_images/marker-icon_aqua.png'});
...
for file in os.listdir(directory_talks):
    with open(directory_talks+"/"+file, 'r') as f:
        ...
        if file.endswith((".md")):
            ...
            ###
            addr_start = lines.find('address: "') + 10 #look for address
            addr_trim = lines[addr_start:]
            addr_end = addr_trim.find('"')
            address = addr_trim[:addr_end]
            ###
            if not address:
                position = geolocator.geocode(location, language='en')
            else:
                position = geolocator.geocode(address, language='en')
