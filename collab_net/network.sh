...
do
	...
	pubtype=$(grep -h "excerpt" $filename | cut -d \: -f 2)
	if (echo $pubtype | grep -qi "preprint"); then
	echo $(grep -h "preprinturl" $filename | cut -d \' -f 2) >> $OUTPUT_URL
	else echo $(grep -h "paperurl" $filename | cut -d \' -f 2) >> $OUTPUT_URL	#extracts the url of the article from the file
	fi
done
